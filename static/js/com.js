...
//评分
function BookshowVote(vote, id) {
    if (!vote) { vote = 0; }
    $("div.book-star-vote").find("li.book-current-rating").css("width", 20 * vote + "px");
    var $score = $("div.book-star-vote").find("b");
    $score.text(vote);

    var $container = $score.closest('.score-container');
    $container.addClass('bubbling');
    setTimeout(() => $container.removeClass('bubbling'), 600); 

    $("p.reviews").find("span.turnout").text(id);
}
